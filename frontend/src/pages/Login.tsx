  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ok) {
        ...
        const isAdmin = currentUser && (currentUser.role === 'super_admin' || currentUser.role === 'admin' || currentUser.role === 'product_admin')
        navigate(isAdmin ? '/cms/dashboard' : '/checkout')
      } else {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-1 text-center text-xs text-gray-500">
          Are you an admin?{' '}
          <Link to="/cms/login" className="font-medium text-primary-600 hover:text-primary-500">
            Go to Admin Login
          </Link>
        </p>
      </div>
