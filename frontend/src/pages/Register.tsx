...
import { Eye, EyeOff, Mail, Lock, User, Phone, AlertTriangle } from 'lucide-react'
import { useAuth } from '../context/AuthContext'

const Register = () => {
  const navigate = useNavigate()
  const { registerWithDetails, isLoading } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If server provided field-specific validation errors, surface them inline
      if (error?.details && Array.isArray(error.details)) {
        const serverErrors: Record<string, string> = {}
        error.details.forEach((d: any) => {
          if (d?.param && d?.msg) {
            serverErrors[d.param] = d.msg
          }
        })
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          {submitError && (
            <div className="mb-4 flex items-start rounded-md border border-red-300 bg-red-50 p-3 text-red-700">
